...
function sortAlphabetical($a, $b) {     // (Array) $a = current; $b = follwing value
  ...
  $a = iconv("UTF-8", "ASCII//TRANSLIT", GeneralFunctions::getLocalized($a['localization'],'title'));//GeneralFunctions::getLocalized($a['localization'],'title');//
  $a = strtolower($a);
  $a = preg_replace('#[^-a-z0-9_ ]+#', '', $a);
  $b = iconv("UTF-8", "ASCII//TRANSLIT", GeneralFunctions::getLocalized($b['localization'],'title'));//GeneralFunctions::getLocalized($b['localization'],'title');//
  $b = strtolower($b);
