...
if(is_file(dirname(__FILE__).'/../../../pages/'.$categoryPath.$page.'.php')) {
  $question = '<h1 class="red">'.$langFile['deletePage_question_part1'].' &quot;<span style="color:#000000;">'.strip_tags(getLocalized($pageContent,'title')).'</span>&quot; '.$langFile['deletePage_question_part2'].'</h1>';
...
if($asking && is_file(dirname(__FILE__).'/../../../pages/'.$categoryPath.$page.'.php')) {
  ...
  if(@unlink(dirname(__FILE__).'/../../../pages/'.$categoryPath.$page.'.php')) {
    ...
    StatisticFunctions::saveTaskLog(2,strip_tags(getLocalized($pageContent,'title'))); // <- SAVE the task in a LOG FILE
