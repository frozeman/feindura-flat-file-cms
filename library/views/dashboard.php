    ...
    echo '<h2>'.$langFile['DASHBOARD_TITLE_STATISTICS_LASTEDITED'].'</h2>';    
      ...
      foreach($pages as $page) {
        if($page['lastSaveDate'] != 0) {
          echo '<tr class="'.$rowColor.'"><td style="font-size:11px;text-align:left;"><b>'.StatisticFunctions::formatDate(StatisticFunctions::dateDayBeforeAfter($page['lastSaveDate'])).'</b> '.StatisticFunctions::formatTime($page['lastSaveDate']).'</td><td><a href="?category='.$page['category'].'&amp;page='.$page['id'].'" class="blue">'.strip_tags(GeneralFunctions::getLocalized($page,'title')).'</a></td></tr>';        
          // change row color
          $rowColor = ($rowColor == 'light') ? 'dark' : 'light';    
          // count
