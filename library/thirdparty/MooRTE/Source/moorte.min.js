...
MooRTE.Range={create:function(a){var b=window.document.selection||window.getSelection();if(!b||b.getRangeAt&&!b.rangeCount)return null;console.log(1);return MooRTE.ranges[a||"a1"]=b.getRangeAt?b.getRangeAt(0):b.createRange()},get:function(a,b){b||(b=MooRTE.Range.create());switch(a){case "text":return b.text||b.toString();case "node":return b.cloneContents?b.cloneContents():new Element("div",{html:b.htmlText});default:case "html":var c=b.htmlText;if(!c){c=b.cloneContents();MooRTE.Range.content.empty().appendChild(c);
...
removeEvents:function(a,b){Object.each(a.retrieve("rteEvents",{}),function(c,d){a.removeEvent(d,c)});b&&a.eliminate("rteEvents")},eventHandler:function(a,b,c){var d=MooRTE.Elements[c][a];switch(typeOf(d)){case "function":d.call(b,c,a);break;case "array":d=Array.clone(d).each(function(f){MooRTE.eventHandler(f,b,c)});break;case "object":Object.every(Object.clone(d),function(f,e){var i=Array.from(f).append([c,a]);MooRTE.Utilities[e].apply(b,i)});break;case "string":a=="source"&&a.substr(0,2)!="on"?MooRTE.Range.wrapText(d,
b):MooRTE.Utilities.eventHandler(d,b,c)}},addElements:function(a,b,c){MooRTE.btnVals.args||MooRTE.btnVals.combine(["args","shortcut","element","onClick","img","onLoad","source","contains"]);if(b){if(Type.isArray(b)){var d=b[1];b=b[0]}}else b=MooRTE.activeBar.getFirst();c||(c={});if(Type.isString(a)){a=a.replace(/'([^']*)'|"([^"]*)"|([^{}:,\][\s]+)/gm,"'$1$2$3'");a=a.replace(/((?:[,[:]|^)\s*)('[^']+'\s*:\s*'[^']+'\s*(?=[\],}]))/gm,"$1{$2}");for(a=a.replace(/((?:[,[:]|^)\s*)('[^']+'\s*:\s*{[^{}]+})/gm,
"$1{$2}");a!=(a=a.replace(/((?:[,[]|^)\s*)('[^']+'\s*:\s*\[(?:(?=([^\],\[]+))\3|\]}|[,[](?!\s*'[^']+'\s*:\s*\[([^\]]|\]})+\]))*\](?!}))/gm,"$1{$2}")););a=JSON.decode("["+a+"]")}var f=[],e=0;do{if(f.length){a=f;f=[]}Array.from(a).each(function(g){switch(typeOf(g)){case "string":f.push(g);break;case "object":Object.each(g,function(l,o){f.push(Object.set(o,l))});break;case "array":g.each(function(l){f.push(l)});e=g.length}})}while(e);var i=[],j=b.hasClass("MooRTE")?b:b.getParent(".MooRTE");f.each(function(g){if(Type.isObject(g)){var l=
Object.values(g)[0];g=Object.keys(g)[0]}var o=".rte"+g.replace(".",".rte")+(c.className?"."+c.className.replace(" ","."):""),m=b["get"+({before:"Previous",after:"Next",top:"First"}[d]||"Last")](o);g=g.split(".")[0];if(!m||!c.ifExists){var k=MooRTE.Elements[g],r=k.element&&k.element.toLowerCase()=="textarea",q="text,password,submit,button,checkbox,file,hidden,image,radio,reset".contains(k.type),s=/bold|italic|underline|strikethrough|unlink|(sub|super)script|insert(un)?orderedlist|justify(left|full|right|center)/i.test(g),
v=Object.append({href:"javascript:void(0)",unselectable:q||r?"off":"on",title:g+(k.shortcut?" (Ctrl+"+k.shortcut.capitalize()+")":""),styles:k.img?isNaN(k.img)?{"background-image":"url("+k.img+")"}:{"background-position":"0 "+-20*k.img+"px"}:{},events:{mousedown:function(n){MooRTE.activeBar=j;var t=j.retrieve("source"),u=j.retrieve("fields"),p=MooRTE.Range.parent();if(Browser.webkit&&p.nodeType==3)p=p.parentElement;if(!(u.contains(MooRTE.activeField)&&MooRTE.activeField.contains(p)))(MooRTE.activeField=
u[0]).focus();if(n&&n.stop)q||r?n.stopPropagation():n.stop();!k.onClick&&!t&&(!k.element||k.element=="a")?MooRTE.Utilities.exec(k.args||g):MooRTE.Utilities.eventHandler(t||"onClick",this,g)}}},k);MooRTE.btnVals[k.element?"include":"erase"]("href").each(function(n){delete v[n]});m=(new Element((q&&!k.element?"input":k.element||"a")+o,v)).inject(b,d);if(k.onUpdate||s)j.retrieve("update",{value:[],state:[],custom:[]})[/font(name|size)|(back|fore|hilite)color/i.test(g)?"value":s?"state":"custom"].push([g,
m,k.onUpdate]);if(k.shortcut)j.retrieve("shortcuts",{})[k.shortcut]=g;MooRTE.Utilities.eventHandler("onLoad",m,g)}if((l=l||k&&k.contains)&&!(c.ifExists=="stop"&&!k))MooRTE.Utilities.addElements(l,m,c.inherit?c:{});m.removeClass("rteHide");i.push(m)});return i[1]?i:i[0]},tabs:function(a){var b=Array.from(arguments),c=b.splice(-2).shift(),d=b[2]||{},f=MooRTE.Tabs[a];if(f)Object.each(f,function(e){e[0]&&e[0].removeClass("rteSelected");e[1]&&e[1].addClass("rteHide");d.onHide&&MooRTE.Utilities.eventHandler(d.onHide,
...
styles:["/siteroller/classes/colorpicker/Source/ColorRoller.js"],onComplete:function(){}})},onClick:function(){}},div:{element:"div",title:""},Toolbar:{element:"div",title:""}};
