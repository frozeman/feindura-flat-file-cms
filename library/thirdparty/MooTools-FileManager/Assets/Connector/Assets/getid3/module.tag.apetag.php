	...
	function getid3_apetag(&$fd, &$ThisFileInfo, $overrideendoffset=0) {
		...
		for ($i = 0; $i < $thisfile_ape['footer']['raw']['tag_items']; $i++) {
			...
			if (strstr(substr($APEtagData, $offset), "\x00") === false) {
				$ThisFileInfo['error'][] = 'Cannot find null-byte (0x00) separator between ItemKey #'.$i.' and value. ItemKey starts '.$offset.' bytes into the APE tag, at file offset '.($thisfile_ape['tag_offset_start'] + $offset);
				return false;
			}
