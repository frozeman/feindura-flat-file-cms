<?php
/**
 * ITALIAN (IT) addon language file
 *
 * NEEDS a RETURN $addonLangFile; at the END
 *
 *
 * Every addon language file has to have:
 *    - $addonLangFile['feinduraAddon_title']        = 'Exampletitle';
 *    - $addonLangFile['feinduraAddon_description']  = 'This is an example addon description.';
 *
 *
 * @package [Addons]
 * @subpackage exampleAddon
 */


/* ADDON ************************************************************************************ */

$addonLangFile['feinduraAddon_title']        = 'Esempio addon';
$addonLangFile['feinduraAddon_description']  = 'Questo addon è solo per mostrare come si potrebbe rendere il vostro addons proprio!';

/* TEXTS ************************************************************************************ */

$addonLangFile['exampleText']       = 'Questo testo ad esempio proviene dal file lingua italiana questo componente aggiuntivo.';

// -----------------------------------------------------------------------------------------------
// RETURN ****************************************************************************************
// -----------------------------------------------------------------------------------------------
return $addonLangFile;

?>